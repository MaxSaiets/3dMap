...
_preloaded_pois: Optional[gpd.GeoDataFrame] = None
...
def load_all_data() -> None:
    ...
    global _preloaded_green, _preloaded_pois, _is_loaded
    ...
    # 5. POI
    print("[preload] Завантаження POI...")
    t_p_start = time.perf_counter()
    _preloaded_pois = osm.get_data_by_custom_criteria(
    ...
    _preloaded_pois = _preloaded_pois if _preloaded_pois is not None else gpd.GeoDataFrame()
    _preloaded_pois = _preloaded_pois[_preloaded_pois.geometry.notna()] if not _preloaded_pois.empty else _preloaded_pois
    _preloaded_pois = _preloaded_pois[_preloaded_pois.geom_type.isin(["Point", "MultiPoint"])] if not _preloaded_pois.empty else _preloaded_pois
    t_p_end = time.perf_counter()
    print(f"[preload] POI: {len(_preloaded_pois)} за {(t_p_end - t_p_start):.2f}s")
    
    # Звільняємо OSM об'єкт
    del osm
    
    # Проекція в UTM
    print("[preload] Проекція в UTM...")
    import osmnx as ox
    ...
    if not _preloaded_buildings.empty:
        with warnings.catch_warnings():
            ...
            _preloaded_buildings = ox.project_gdf(_preloaded_buildings)
    
    if not _preloaded_water.empty:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DeprecationWarning)
            _preloaded_water = ox.project_gdf(_preloaded_water)
    
    if not _preloaded_roads.empty:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DeprecationWarning)
            _preloaded_roads = ox.project_gdf(_preloaded_roads)
    
    if not _preloaded_green.empty:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DeprecationWarning)
            _preloaded_green = ox.project_gdf(_preloaded_green)
    
    if not _preloaded_pois.empty:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DeprecationWarning)
            _preloaded_pois = ox.project_gdf(_preloaded_pois)
    
    t_proj_end = time.perf_counter()
    print(f"[preload] Проекція завершена за {(t_proj_end - t_proj_start):.2f}s")
    
    # Створюємо spatial index
    print("[preload] Створення spatial index...")
    t_idx_start = time.perf_counter()
    if not _preloaded_buildings.empty:
        _preloaded_buildings.sindex  # Створює spatial index
    if not _preloaded_water.empty:
        _preloaded_water.sindex
    if not _preloaded_roads.empty:
        _preloaded_roads.sindex
    if not _preloaded_green.empty:
        _preloaded_green.sindex
    if not _preloaded_pois.empty:
        _preloaded_pois.sindex
    t_idx_end = time.perf_counter()
    print(f"[preload] Spatial index створено за {(t_idx_end - t_idx_start):.2f}s")
    
    _is_loaded = True
    t_total = time.perf_counter() - t0
    print("=" * 80)
    print(f"[preload] ВСІ ДАНІ ЗАВАНТАЖЕНО за {t_total:.2f}s")
    print(f"  Будівлі: {len(_preloaded_buildings)}")
    print(f"  Вода: {len(_preloaded_water)}")
    print(f"  Дороги: {len(_preloaded_roads)}")
    print(f"  Зелені зони: {len(_preloaded_green)}")
    print(f"  POI: {len(_preloaded_pois)}")
...
def get_extras_for_bbox(
    ...
    """
    Отримує extras (green, pois) для вказаного bbox.
    ...
    elif not _preloaded_pois.empty:
        target_crs = _preloaded_pois.crs
    else:
        target_crs = None
    ...
    if not _preloaded_pois.empty:
        try:
            possible_matches_index = list(_preloaded_pois.sindex.intersection(bbox_bounds))
            if possible_matches_index:
                pois_result = _preloaded_pois.iloc[possible_matches_index].copy()
                pois_result = pois_result[pois_result.geometry.intersects(bbox_geom)]
        except Exception as e:
            print(f"[WARN] Помилка вибору POI: {e}")
    
    return green_result, pois_result
