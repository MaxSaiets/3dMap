                ...
                try:
                    ...
                    if bridge_area is not None and not bridge_area.is_empty:
                        ...
                        if final_height < 4.0 and layer_val >= 1:
                            final_height = 4.0
                        bridges.append((bridge_line, bridge_area, final_height, is_over_water, layer_val))
                except Exception:
                    ...
                    try:
                        ...
                        if final_height < 4.0 and layer_val >= 1:
                            final_height = 4.0
                        bridges.append((geom, geom.buffer(float(bridge_buffer_m)), final_height, is_over_water, layer_val))
                    except Exception:
                        pass
...
def build_road_polygons(
    ...
    extra_buffer_m: float = 0.0,  # Додатковий буфер для маски вирізання (метри)
) -> Optional[object]:
    """
    ...
    Args:
        extra_buffer_m: Додатковий буфер з кожного боку дороги (для створення "узбіччя" при вирізанні з парків)
    """
    if G_roads is None:
        return None
    ...
    def get_width(row):
        ...
        # Ширина для буфера - це половина загальної ширини + додатковий буфер
    ...
    for _, row in gdf_edges.iterrows():
        try:
            ...
            if not geom.is_valid:
                geom = geom.buffer(0)
            # УВАГА: row.width тепер вже містить радіус буфера (половина ширини + extra_buffer)
            # Тому не ділимо на 2
            ...
            try:
                    ...
                    if terrain_provider is not None:
                        ...
                        if is_bridge:
                            ...
                            if hasattr(terrain_provider, 'original_heights_provider') and terrain_provider.original_heights_provider is not None:
                                ...
                                # ВИПРАВЛЕННЯ: Використовуємо стабільне заглиблення замість adaptive_embed
                                # КРИТИЧНО: Додаємо GLOBAL_ROAD_LIFT до base_nonbridge для синхронізації стиків
                                stable_embed = float(re) if float(re) > 0 else 0.1
                                GLOBAL_ROAD_LIFT = 0.05  # Глобальний bias для всіх доріг (має співпадати з bias у блоці else)
                                base_nonbridge = ground_z_values - stable_embed + GLOBAL_ROAD_LIFT
                            ...
                            # --- ВИПРАВЛЕННЯ: Видалено adaptive_embed ---
                            # Використовуємо однакове заглиблення для всіх частин дороги
                            # Це гарантує, що всі стики будуть ідеальними без розривів
                            stable_embed = float(re) if float(re) > 0 else 0.1
                            
                            # Глобальний bias для синхронізації стиків (має співпадати з GLOBAL_ROAD_LIFT у блоці мостів)
                            GLOBAL_ROAD_LIFT = 0.05  # 5 см для уникнення z-fighting та синхронізації стиків
                            
                            road_z = ground_z_values + old_z - stable_embed + GLOBAL_ROAD_LIFT
                ...
                for b_line, b_area, b_h, b_water, b_layer in bridges_layer2plus:
                    if b_area is None or b_area.is_empty:
                        continue
                    try:
                        inter = poly.intersection(b_area)
                    except Exception:
                        continue
                    for g in _iter_polys(inter):
                        if g is None or g.is_empty:
                            continue
                        if float(getattr(g, "area", 0.0) or 0.0) < 0.001:
                            continue
